...
Language::Language(const std::string& langName): _name(langName) {}

bool Language::isUnknown() const
{
	return _name.empty();
}
bool Language::isKnown() const
{
	return !isUnknown();
}
bool Language::isModuleCountSet() const
{
	return _moduleCount >= 0;
}
bool Language::isBytecode() const
{
	return _bytecode;
}

void Language::setName(const std::string& n)
{
	_name = n;
}
void Language::setIsUnknown()
{
	_name.clear();
}
void Language::setModuleCount(unsigned c)
{
	_moduleCount = c;
}
void Language::setIsBytecode(bool b)
{
	_bytecode = b;
}

unsigned Language::getModuleCount() const
{
	return _moduleCount;
}
