...
Pattern::Match::Match(
	...
	eType type):
	_offset(offset),
	_address(address), _size(size), _entrySize(entrySize), _type(type)
{}
...
{
	return getOffset() == val.getOffset() && getAddress() == val.getAddress() && getSize() == val.getSize()
		&& getEntrySize() == val.getEntrySize() && _type == val._type;
}
...
Pattern::Pattern(
	...
	eEndian endian):
	_name(name),
	_description(description), _yaraRuleName(yaraRuleName), _type(type), _endian(endian)
{}
...
{
	return getName() == val.getName() && getDescription() == val.getDescription()
		&& getYaraRuleName() == val.getYaraRuleName() && _type == val._type && _endian == val._endian
		&& matches == val.matches;
}
